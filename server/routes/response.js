...
router.get('/entities', async (req, res) => {
	const entities = await Response.find({ _id: req.body.id }).select({ entities: 1 })
...
router.post('/keyword', async (req, res) => {
	const getKeyword = await Response.updateOne({ _id: req.body.id }, { $push: { entities: req.body.keyword } })
