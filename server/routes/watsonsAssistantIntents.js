...
router.get('/', async (req, res) => {
  try {
    ...
    const assistant = await getWatsonAssistant();
    if (!assistant) return res.status(400).send('Watson credentials not configured.');
    const response = await assistant.listExamples({ workspaceId: workspace_id, intent });
...
router.post('/', async (req, res) => {
  try {
    ...
    const response = await assistant.createIntent({ workspaceId: workspace_id, intent, description, examples });
    res.send(response.result);
  } catch (err) {
    res.status(400).send(err.message || err);
  }
